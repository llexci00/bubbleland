...
(function () {
  ...
  const TM_MAX = 24 * 3600, TM_STUDY_MAX = 24 * 3600 * 1000, TM_KEY = "bubbleland.study";   // 최대 24시간
  ...
  function startStudy() {
    ...
    tmStIv = setInterval(() => {
      tmStMs = Date.now() - tmStBase;
      if (tmStMs >= TM_STUDY_MAX) { tmStMs = TM_STUDY_MAX; document.getElementById("tm-study-disp").textContent = fmtHMS(tmStMs / 1000); stopStudy(); return; }   // 24시간 상한 → 자동 정지·기록
      document.getElementById("tm-study-disp").textContent = fmtHMS(tmStMs / 1000);
    }, 200);
  }
